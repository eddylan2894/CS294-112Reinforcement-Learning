...
class QLearner(object):
  ...
  def __init__(
    ...
    self.kp = tf.placeholder(shape = None, dtype=tf.float32)
    ...
    q_f = q_func(obs_t_float, self.num_actions, scope="q_func", keep_prob=self.kp, reuse=False)
    self.action_q = tf.argmax( q_f, 1, output_type = tf.int32)
    # add q(s,a)
    if double_q:
      q_f_n = q_func(obs_tp1_float, self.num_actions, scope = "q_func",keep_prob=1.0, reuse = True)

    self.q_target = q_func(obs_tp1_float, self.num_actions, scope="q_target", keep_prob= 1.0, reuse=False)
  ...
  def step_env(self):
    ...
    if (self.model_initialized == False):
      ...
      action = self.session.run(self.action_q, feed_dict={self.obs_t_ph : lo_c[None,:], self.kp: self.exploration.value(self.t)})
      action = action[0]
