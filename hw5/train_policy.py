...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    x = build_mlp(x, output_size, 'mlp', n_layers, size, activation, output_activation, regularizer)
    x = tf.reshape(x, [-1, history, output_size])
    cell = tf.nn.rnn_cell.GRUCell(output_size, activation)
    output, state = tf.nn.dynamic_rnn(cell, x, initial_state=h, time_major=False)
    return state, h
...
class Agent(object):
    def __init__(self, computation_graph_args, sample_trajectory_args, estimate_return_args):
        ...
        self.side = sample_trajectory_args['side']
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        """
        env.reset_task(is_evaluation=is_evaluation, side= self.side)
        stats = []
        #====================================================================================#
        #                           ----------PROBLEM 1----------
        #====================================================================================#
        ...
        while True:
            ...
            # YOUR CODE HERE
            meta_ob = np.concatenate((ob,ac,[rew],[done]), 0)
